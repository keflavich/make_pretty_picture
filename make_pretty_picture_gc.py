...
import os
os.chdir('/Volumes/disk4/gc/WISE')

if not 'b2' in locals():
    import pyfits
    b2 = pyfits.open('WISE_B2_GC_mosaic_bgmatch.fits')
    b3 = pyfits.open('WISE_B3_GC_mosaic_bgmatch.fits')
    b4 = pyfits.open('WISE_B4_GC_mosaic_bgmatch.fits')
    v2 = pyfits.open('v2.0_ds2_GC_WISEpix.fits')
    v2[0].data[3300:,:] = 0
    v2[0].data[:2150,:] = 0
...
# or do this: 
b1min = 100 ; b1max=2700; b1scale=3.0
b2min = 43 ; b2max=2900; b2scale=3.0
b3min = 900; b3max=4500; b3scale=3.0 #1.25
b4min = 285; b4max=2300; b4scale=3.0 #0.86
def linearize(x):
    if np.isscalar(x):
        return x
    else:
...
if not 'v2scaled' in locals():
    if test:
        ...
        myshape = (2700,7200)
        myslice = slice(1350,4050,None),slice(400,7600,None)
...
savefig(prefix+"GC_WISE_mosaic.png",dpi=300)
imshow(v2img)
print "Saving RGB +bolo mosaic ",time.time()-t0
savefig(prefix+"GC_WISE_mosaic_bolocam.png",dpi=300)
draw()
...
savefig(prefix+"GC_WISE_mosaic_bolocam_try2.png",dpi=300)
...
savefig(prefix+'GC_bolocam_orange.png',dpi=300)
colorbar()
draw()
...
im1.save(prefix+'GC_4096sq_WISE_mosaic.png')
#rgb_pil = ((1-rgb[:,:,:3])*(2**8))
#rgb_pil -= (256*rgb[:,:,3])[:,:,newaxis]
#rgb_pil[rgb_pil>255] = 255
#rgb_pil = rgb_pil.astype('uint8')
#im1 = PIL.Image.fromarray(rgb_pil)
#im1.save(prefix+'GC_4096sq_WISE_mosaic_try2.png')
...
wbackground.save(prefix+'GC_4096sq_WISE_mosaic_whitebg.png')
...
print "Saving WISE mosaic ",time.time()-t0
im2.save(prefix+'GC_4096sq_WISE_mosaic_rotated.png')
kbackground2 = PIL.Image.new("RGB", im2.size, (0, 0, 0))
kbackground2.paste(im2, mask=im2.split()[3])
print "Saving WISE mosaic with black bg ",time.time()-t0
kbackground2.save(prefix+'GC_4096sq_WISE_mosaic_rotated_blackbg.png')
kbackground2_small = kbackground2.resize([s/4 for s in kbackground2.size])
kbackground2_small.save(prefix+'GC_4096sq_WISE_mosaic_rotated_blackbg_small.png',quality=10)
print "Saving Bolocam + WISE mosaic with black bg (PIL) ",time.time()-t0
kbackground2_contrast = ImageOps.autocontrast(kbackground2)
kbackground2_contrast.save(prefix+'GC_4096sq_WISE_mosaic_rotated_blackbg_contrast.png')
kbackground2_bright = ImageEnhance.Brightness(kbackground2_contrast).enhance(1.5)
kbackground2_bright.save(prefix+'GC_4096sq_WISE_mosaic_rotated_blackbg_contrast_bright.png')
kbackground2_small_bright = kbackground2_bright.resize([s/4 for s in kbackground2.size])
kbackground2_small_bright.save(prefix+'GC_4096sq_WISE_mosaic_rotated_blackbg_bright_small.png',quality=10)
kbackground2_bright.paste(boloim, mask=boloim.split()[3])
kbackground2_bright.save(prefix+'GC_4096sq_WISE_bolo_mosaic_rotated_blackbg.png')
kbackground2_small = kbackground2_bright.resize([s/4 for s in kbackground2.size])
kbackground2_small.save(prefix+'GC_4096sq_WISE_bolo_mosaic_rotated_blackbg_small.png',quality=10)
