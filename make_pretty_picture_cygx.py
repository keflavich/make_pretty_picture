...
if not 'b1' in locals():
    import pyfits
    b1 = pyfits.open('WISE_B1_CygX_mosaic_bgmatch.fits')
    b3 = pyfits.open('WISE_B3_CygX_mosaic_bgmatch.fits')
    b4 = pyfits.open('WISE_B4_CygX_mosaic_bgmatch.fits')
    v2 = pyfits.open('cygx_v2_mosaic_WISEpix.fits')
...
# or do this: 
b1min = 10 ; b1max=1000; b1scale=3.0
b3min = 450; b3max=8000; b3scale=3.0 #1.25
b4min = 130; b4max=1000; b4scale=3.0 #0.86
def linearize(x):
    if np.isscalar(x):
        return x
    else:
...
if not 'v2scaled' in locals():
    if test:
        ...
        v2d = v2[0].data
        v2d[v2d!=v2d] = 0
        v2smooth = np.fft.fftshift( np.fft.ifft2( np.fft.fft2(v2d*(v2d>display_cutoff)) * np.fft.fft2(AG_fft_tools.smooth_tools.make_kernel([4096,4096])) ) ).real
        v2scaled = v2scale(v2smooth)#AG_fft_tools.smooth(v2[0].data*(v2[0].data>0.025)))
...
autumn_transparent._lut[:,3] *= np.sin(np.linspace(0.1,1.0,autumn_transparent.N+3)*pi/2)**2
...
savefig(prefix+"cygx_WISE_mosaic.png",dpi=300)
imshow(v2img)
print "Saving RGB +bolo mosaic ",time.time()-t0
savefig(prefix+"cygx_WISE_mosaic_bolocam.png",dpi=300)
draw()
...
savefig(prefix+"cygx_WISE_mosaic_bolocam_try2.png",dpi=300)
...
savefig(prefix+'cygx_bolocam_orange.png',dpi=300)
colorbar()
draw()
...
im1.save(prefix+'cygx_4096sq_WISE_mosaic.png')
#rgb_pil = ((1-rgb[:,:,:3])*(2**8))
#rgb_pil -= (256*rgb[:,:,3])[:,:,newaxis]
#rgb_pil[rgb_pil>255] = 255
#rgb_pil = rgb_pil.astype('uint8')
#im1 = PIL.Image.fromarray(rgb_pil)
#im1.save(prefix+'cygx_4096sq_WISE_mosaic_try2.png')
...
wbackground.save(prefix+'cygx_4096sq_WISE_mosaic_whitebg.png')
...
print "Saving WISE mosaic ",time.time()-t0
im2.save(prefix+'cygx_4096sq_WISE_mosaic_rotated.png')
kbackground2 = PIL.Image.new("RGB", im2.size, (0, 0, 0))
kbackground2.paste(im2, mask=im2.split()[3])
print "Saving WISE mosaic with black bg ",time.time()-t0
kbackground2.save(prefix+'cygx_4096sq_WISE_mosaic_rotated_blackbg.png')
kbackground2_small = kbackground2.resize([s/4 for s in kbackground2.size])
kbackground2_small.save(prefix+'cygx_4096sq_WISE_mosaic_rotated_blackbg_small.png',quality=10)
print "Saving Bolocam + WISE mosaic with black bg (PIL) ",time.time()-t0
kbackground2_contrast = ImageOps.autocontrast(kbackground2)
kbackground2_contrast.save(prefix+'cygx_4096sq_WISE_mosaic_rotated_blackbg_contrast.png')
kbackground2_bright = ImageEnhance.Brightness(kbackground2_contrast).enhance(1.5)
kbackground2_bright.save(prefix+'cygx_4096sq_WISE_mosaic_rotated_blackbg_contrast_bright.png')
kbackground2_small_bright = kbackground2_bright.resize([s/4 for s in kbackground2.size])
kbackground2_small_bright.save(prefix+'cygx_4096sq_WISE_mosaic_rotated_blackbg_bright_small.png',quality=10)
kbackground2_bright.paste(boloim, mask=boloim.split()[3])
kbackground2_bright.save(prefix+'cygx_4096sq_WISE_bolo_mosaic_rotated_blackbg.png')
kbackground2_small = kbackground2_bright.resize([s/4 for s in kbackground2.size])
kbackground2_small.save(prefix+'cygx_4096sq_WISE_bolo_mosaic_rotated_blackbg_small.png',quality=10)
